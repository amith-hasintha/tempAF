...
const TransactionItem = () => {
  ...
  const handleUpdate = async (id) => {
    try {
      ...
      setTransactions(transactions.map(transaction => (transaction._id === id ? res.data.transaction : transaction)));
      setEditingId(null);
    } catch (error) {
    ...
    <>
      ...
      <div className="transactionList">
        <div className="header12">
          ...
          <button className="addButton" onClick={() => navigate('/add-transaction')}>+ Add Transaction</button>
        </div>
